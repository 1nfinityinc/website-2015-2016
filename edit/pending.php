...
foreach($users as $key => $value)
{
	$query = 'SELECT * FROM purchases WHERE user_uuid="' . $value . '"';
		...
		if ($loop > 0 ){
			$loop = 0;
			print '<div style="background-color: lightblue; padding: 10px; margin: 10px; border-radius: 10px;">
			...
			if (@mysql_query ($query)) {
				...
				WHILE($row = mysql_fetch_array ($results)) { // this loops through all the rows that were pulled from the DB
					...
					if ($row['pending'] == 0) 
					{
						...
						<a href="?pid=' . $row['uuid'] . '&type=cancel"><i class="fa fa-times" style="color: red;"></i></h3></a>';
			...
			if ($loop = 0) {
			   print '<div class="text-center" style="background-color: green; padding: 10px; margin: 10px; border-radius: 10px;"><h1 style="color: black">NO PENDING PURCHASEs</h1></div>';
			}
			print $loop;
		}
	}
